    ...
    WrappedNativeReceiver
{
    ...
    function _transferFeesAndFunds(address currency_, address from_, address to_, uint256 amount_) internal {
        ...
        if (currency_ == WRAP_NATIVE) {
            ...
            _transferCurrency(NATIVE_TOKEN, address(this), _protocolFeeRecipient, fee);
        } else {
    ...
    function _validateBasicOrderInfo(OrderStructs.Maker calldata makerAsk) private view {
        ...
        if (!hasRole(LibRoles.CURRENCY_ROLE, makerAsk.currency)) revert Exchange__InvalidCurrency();

        if (!hasRole(LibRoles.COLLECTION_ROLE, makerAsk.collection)) revert Exchange__InvalidCollection();
    ...
    function _authorizeUpgrade(address newImplementation) internal override onlyRole(LibRoles.UPGRADER_ROLE) {}
}
